...
CREATE DATABASE "service-typescript-node-express"
    WITH 
    OWNER = postgres
    ENCODING = 'UTF8'
...
\c "service-typescript-node-express";
...
CREATE TABLE cart_operations (
    id SERIAL PRIMARY KEY,
    cart_id INTEGER REFERENCES carts(id) ON DELETE CASCADE,
    operation_type VARCHAR(50) NOT NULL, -- 'add_item', 'remove_item', 'update_quantity', 'apply_coupon'
    details JSONB,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
...
GRANT ALL PRIVILEGES ON DATABASE "service-typescript-node-express" TO postgres;
